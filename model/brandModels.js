...
const phoneModel = {
    addbrand(data,cb) {
        MongoClient.connect(url, function(err, client) {
            ...
            let saveDate = {
                phonelogo: data.brandlogo,
               brandname: data.brandname,
            }
            async.series([
                function (callback) {
                  // 查询是否已将添加过
                  db.collection('brand').find({brandname: saveDate.brandname}).count(function(err, num) {
                    if (err) {
                      callback({ code: -101, msg: '查询是否已添加失败' });
                    } else if (num !== 0) {
                      console.log('手机品牌已经添加过了');
                      callback({ code: -102, msg: '手机品牌已经添加过了'});
                    } else {
                ...
                function (callback) {
                  // 写入数据库的操作
                  db.collection('brand').insertOne(saveDate, function(err) {
                    if (err) {
